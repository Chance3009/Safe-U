...
export default function SafetyScreen() {
    const [currentScreen, setCurrentScreen] = useState<'home' | 'setup' | 'waiting' | 'active'>('home');
    ...
    const checkInTimer = useRef<NodeJS.Timeout | null>(null);

    const [friends] = useState<Friend[]>([
        { id: '1', name: 'Sarah Chen', avatar: '👩‍🦰', isOnline: true, isSelected: false },
        { id: '2', name: 'Mike Johnson', avatar: '👨‍🦱', isOnline: true, isSelected: false },
        { id: '3', name: 'Emma Davis', avatar: '👩‍🦳', isOnline: false, isSelected: false },
        { id: '4', name: 'Alex Kim', avatar: '👨‍🦲', isOnline: true, isSelected: false },
        { id: '5', name: 'Jordan Lee', avatar: '👩‍🦱', isOnline: false, isSelected: false },
    ]);
            ...
            checkInTimer.current = setInterval(() => {
                setNextCheckIn(prev => {
                    if (prev <= 1) {
                        ...
                        Alert.alert(
                            'Missed Check-in',
                            'Alert sent to ' + selectedFriends.map(id =>
                                friends.find(f => f.id === id)?.name
                            ).join(', '),
                            [{ text: 'Check-in Now', onPress: () => setNextCheckIn(checkInInterval * 60) }]
                        );
    ...
    }, [isFriendWalkActive, currentScreen, selectedFriends, friends, checkInInterval]);

    const handleStartFriendWalk = () => {
        if (fromLocation.name !== 'Current Location' && toLocation.name !== 'Destination' && selectedFriends.length > 0) {
            ...
            Alert.alert('Missing Information', 'Please select both locations and at least one friend.');
        }
    };
    ...
    const handleCheckIn = () => {
        setNextCheckIn(checkInInterval * 60);
        Alert.alert('Check-in Sent', 'Your friends have been notified that you are safe.');
    };
        ...
        return (
            <ScrollView style={[styles.container, { backgroundColor: isDark ? '#000000' : '#f5f5f5' }]}>
                ...
                <View style={styles.cardsContainer}>
                    {/* FriendWalk Card */}
                    <TouchableOpacity
                        style={[styles.safetyCard, { backgroundColor: isDark ? '#1c1c1e' : '#ffffff' }]}
                    ...
                    >
                        ...
                        <Text style={[styles.cardDescription, { color: isDark ? '#999999' : '#666666' }]}>
                    ...
                    {/* Safe-Haven Card */}
                    <TouchableOpacity style={[styles.safetyCard, { backgroundColor: isDark ? '#1c1c1e' : '#ffffff' }]}>
                        <View style={styles.cardHeader}>
            ...
            <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#f5f5f5' }]}>
                    ...
                    <View style={[styles.section, { backgroundColor: isDark ? '#1c1c1e' : '#ffffff' }]}>
                        ...
                        <View style={styles.locationInputs}>
                            ...
                            >
                                ...
                                <View style={styles.locationInputContent}>
                                    <Text style={[styles.locationLabel, { color: isDark ? '#999999' : '#666666' }]}>
                                        From
                                    </Text>
                        ...
                        <View style={styles.routeInfo}>
                            <View style={styles.routeInfoItem}>
                                ...
                                <Text style={[styles.routeInfoText, { color: isDark ? '#ffffff' : '#000000' }]}>
                                    ETA: {getETA()}
                                </Text>
                            </View>
                            <View style={styles.routeInfoItem}>
                        ...
                        <View style={styles.intervalPicker}>
                            <Text style={[styles.intervalLabel, { color: isDark ? '#ffffff' : '#000000' }]}>
                                Check-in Interval:
                            </Text>
                                ...
                                {[5, 10, 15].map((interval) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            ...
                                            {
                                                backgroundColor: checkInInterval === interval ? '#007AFF' : (isDark ? '#333333' : '#f0f0f0'),
                                                borderColor: checkInInterval === interval ? '#007AFF' : 'transparent'
                                            }
                                        ]}
                                    ...
                                    >
                                        <Text style={[
                                            ...
                                            { color: checkInInterval === interval ? 'white' : (isDark ? '#ffffff' : '#000000') }
                                        ]}>
                                            {interval} min
                                        </Text>
                                    </TouchableOpacity>
                            ...
                            {friends.map((friend) => (
                                ...
                                >
                                    ...
                                    <View style={[
                                        ...
                                        { backgroundColor: friend.isOnline ? '#34C759' : '#FF3B30' }
                                    ]} />
                                </TouchableOpacity>
                            ))}
                        </View>
                ...
                >
                    <View style={styles.modalOverlay}>
                        <View style={[styles.locationPickerModal, { backgroundColor: isDark ? '#1c1c1e' : '#ffffff' }]}>
                            <View style={styles.modalHeader}>
                                <Text style={[styles.modalTitle, { color: isDark ? '#ffffff' : '#000000' }]}>
                                    Select {locationPickerType === 'from' ? 'Starting' : 'Destination'} Location
                                </Text>
                            ...
                            <ScrollView style={styles.locationOptions}>
                                <Text style={[styles.locationSectionTitle, { color: isDark ? '#ffffff' : '#000000' }]}>
                                    Recent Locations
                                </Text>
                                {recentLocations.map((location, index) => (
                                    ...
                                    >
                                        <Ionicons name="time" size={20} color="#FF9500" />
                                        <Text style={[styles.locationOptionText, { color: isDark ? '#ffffff' : '#000000' }]}>
                                            {location.name}
                                        </Text>
                                    </TouchableOpacity>
                                ))}
        ...
        return (
            <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#f5f5f5' }]}>
                <View style={styles.waitingHeader}>
                    <Text style={[styles.waitingTitle, { color: isDark ? '#ffffff' : '#000000' }]}>
                        Invites Sent
                    </Text>
                    <Text style={[styles.waitingSubtitle, { color: isDark ? '#999999' : '#666666' }]}>
                ...
                <View style={styles.waitingContent}>
                    <View style={styles.inviteStatus}>
                        ...
                        <Text style={[styles.inviteStatusText, { color: isDark ? '#ffffff' : '#000000' }]}>
                            Invites sent to: {selectedFriends.map(id =>
                                friends.find(f => f.id === id)?.name
                            ).join(', ')}
                        </Text>
                    </View>
    ...
    if (currentScreen === 'active') {
        return (
            <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#f5f5f5' }]}>
                ...
                {/* Timer and Controls */}
                <View style={[styles.activeControls, { backgroundColor: isDark ? '#1c1c1e' : '#ffffff' }]}>
                    ...
                    <View style={styles.controlButtons}>
                        <TouchableOpacity style={styles.controlButton} onPress={handleCheckIn}>
                        ...
                        <TouchableOpacity style={styles.controlButton} onPress={handleEndFriendWalk}>
